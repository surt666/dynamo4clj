...
(defn- insert-event [author category title content]
  (let [seqno (Integer/parseInt (:value (update-item  (get-client)  "seq" "event_count" {:value [1 "add"]})))]
    (insert-item "events" {:priority "NORMAL" :id seqno :author author :category category :title title :content content})))

(comment (time (doall (pmap #(insert-event (get-client) "steen" "events" (str "test" %) "{\"bla\" : 2}") (range 100)))))
